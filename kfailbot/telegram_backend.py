...
class KFailBTelegramBot:
    ...
    def __add_handlers(self):
        ...
        menu = CallbackQueryHandler(
            self.unsubscribe_menu, pattern=f"^{_unsubscribe_prefix}"
        )
    ...
    def unsubscribe_menu_keyboard(lines):
        keyboard = [
            [
                InlineKeyboardButton(
                    f"Line {x}", callback_data=f"{_unsubscribe_prefix}{x}"
                )
                for x in lines
            ]
        ]
        keyboard.append(
            [
                InlineKeyboardButton("All", callback_data=f"{_unsubscribe_prefix}all"),
                InlineKeyboardButton(
                    "Cancel", callback_data=f"{_unsubscribe_prefix}cancel"
                ),
            ]
        )
    ...
    def unsubscribe(self, bot, update):
        ...
        if lines:
            ...
            update.message.reply_text(
                "You are not subscribed to any line, yet. \nIn order to do so, use the command"
                " /subscribe <line>"
            )
        ...
        for recipient in recipients:
            ...
            if not silence.is_effective():
                ...
                self._bot.send_message(
                    chat_id=recipient, text=text, disable_notification=True
                )
    ...
    def mute_menu_keyboard():
        ...
        keyboard = [
            [
                InlineKeyboardButton(f"{x}h", callback_data=f"{_silence_prefix}{x}")
                for x in options
            ]
        ]
        return InlineKeyboardMarkup(keyboard)
